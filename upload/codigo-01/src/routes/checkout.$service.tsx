...
export const Route = createFileRoute("/checkout/$service")({
  validateSearch: (s) => checkoutSearch.parse(s),
  beforeLoad: ({ params, search }) => {
    if (requiresQueue(params.service) && !search.queue) {
      throw redirect({ to: "/fila/$service", params: { service: params.service } });
    }
  },
  head: ({ params }) => {
    const service = getService(params.service);
    const title = service
      ? `Adquirir ${service.shortName} | Clodoaldo Silva`
      : "Checkout | Clodoaldo Silva";
    return {
      meta: [
        { title },
        {
          name: "description",
          content:
            "Finalize sua contratação com Clodoaldo Silva em um fluxo rápido com briefing, upsell e pagamento.",
        },
...
function CheckoutPage() {
  ...
  const { queue: queueId } = Route.useSearch();
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (result.url) {
        ...
        navigate({ to: "/checkout/sucesso", search: { order: result.orderId } as never });
      }
    } catch (e) {
  ...
  return (
    <div className="min-h-screen">
      <Header />
      <main className="mx-auto max-w-3xl px-5 sm:px-8 pt-28 sm:pt-32 pb-20">
        <Link
          to="/"
                ...
                <div className="rounded-2xl border border-border bg-background/40 p-5">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="font-display font-medium mt-1">
                        {upsell.shortName}
                      </div>
            ...
            {step === "contact" && (
              ...
              >
                ...
                <div className="flex gap-3">
                  ...
                  >
                    {submitting ? "Processando..." : service.kind === "ebook" ? "Comprar agora" : "Pagar com Stripe"}
                  </button>
                </div>
                <p className="text-xs text-muted-foreground text-center leading-relaxed">
                  Pagamento seguro processado pela Stripe. Após a confirmação, você receberá o item contratado e eventuais bônus liberados automaticamente.
                </p>
              </form>
            )}
          </div>
