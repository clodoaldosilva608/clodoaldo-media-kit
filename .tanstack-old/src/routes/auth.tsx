...
export const Route = createFileRoute("/auth")({
  ...
  head: () => ({
    meta: [
      { title: "Entrar | Clodoaldo Silva" },
      { name: "description", content: "Acesse sua conta para desbloquear a biblioteca premium Knowledge Hub." },
...
function AuthPage() {
  ...
  useEffect(() => {
    supabase.auth.getSession().then(({ data }) => {
      if (data.session) {
        navigate({ to: redirect ?? "/knowledge" });
      }
    });
